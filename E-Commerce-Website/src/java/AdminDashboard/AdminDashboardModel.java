...
public class AdminDashboardModel {
    ...
    public ArrayList<AdminDashboardProductDetails> getLowStockProductList() throws SQLException {
        ...
        String query = "SELECT Product.product_id, Product.name, SUM(Product_stock.available_quantity) AS total_available_quantity FROM Product LEFT JOIN Product_stock ON Product.product_id = Product_stock.product_id GROUP BY Product.product_id, Product.name ORDER BY total_available_quantity LIMIT 10;";
    ...
    public ArrayList<AdminDashboardOrderDetails> getStatusOrderList() throws SQLException {
        ...
        String pendingOrderQuery = "SELECT order_id, date_time, total_price, status FROM Orders WHERE status = 'Pending' ORDER BY date_time LIMIT 10;";
        String dispatchedOrderQuery = "SELECT order_id, date_time, total_price, status FROM Orders WHERE status = 'Dispatched' ORDER BY date_time LIMIT ? ;";
        String deliveredOrderQuery = "SELECT order_id, date_time, total_price, status FROM Orders WHERE status = 'Delivered' ORDER BY date_time DESC LIMIT ? ;";
        ...
        try{
            ...
            PreparedStatement pstmt1 = connection.prepareStatement(dispatchedOrderQuery);
            
            PreparedStatement pstmt2 = connection.prepareStatement(deliveredOrderQuery);
            
            //Getting the 10 oldest pending orders
            ResultSet rs1 = stmt.executeQuery(pendingOrderQuery);
    ...
    public ArrayList<AdminDashboardProductDetails> getMonthlyTopOrderedProducts() throws SQLException {
        ...
        String query = "SELECT Product.product_id, Product.name, SUM(Orders_Product.quantity) AS total_quantity_sold "
                            +"FROM Orders_Product " 
    ...
    public ArrayList<String> getTotalOrdersAndDeliveredOrdersOfTheMonth() throws SQLException{
        ...
        String totalOrdersQuery = "SELECT COUNT(order_id) FROM Orders WHERE YEAR(date_time) = YEAR(CURRENT_DATE()) AND MONTH(date_time) = MONTH(CURRENT_DATE());";
        String deliveredOrdersQuery = "SELECT COUNT(order_id) FROM Orders WHERE YEAR(date_time) = YEAR(CURRENT_DATE()) AND MONTH(date_time) = MONTH(CURRENT_DATE()) AND status = 'Delivered';";
        ...
        try{
            ...
            ResultSet rs1 = stmt1.executeQuery(totalOrdersQuery);
            rs1.next();
            arrList.set(0, Integer.toString(rs1.getInt("COUNT(order_id)")));
            rs1.close();
            
            ResultSet rs2 = stmt2.executeQuery(deliveredOrdersQuery);
            rs2.next();
